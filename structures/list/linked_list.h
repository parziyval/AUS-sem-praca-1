...
namespace structures
{
	...
	inline LinkedList<T>& LinkedList<T>::operator=(const LinkedList<T>& other)
	{
		if (this != &other)
		{
			...
			while (current != nullptr) { // = kym nie sme na konci
				add(current->accessData()); //pridaj do aktualneho zoznamu prvok z other zoznamu
				current = current->getNext(); //posun sa v other zozname
			}
		}
		return *this;
	...
	inline T LinkedList<T>::removeAt(const int index)
	{
		...
		{
			LinkedListItem<T>* prevItem = getItemAtIndex(index - 1); //item pred tym, ktory sa ma mazat
			deleted = prevItem->getNext();
			prevItem->setNext(deleted->getNext());
			if (index == size_ - 1) //ak sme zmazali posledny
			{
				last_ = prevItem;
			}
		}
	...
	{
		return new LinkedListIterator(nullptr); //nullptr je prvok za prvkom last_
	}
	...
	inline Iterator<T>& LinkedList<T>::LinkedListIterator::operator=(const Iterator<T>& other)
	{
		...
		//kontrola identity je osetrena niekde vraj
