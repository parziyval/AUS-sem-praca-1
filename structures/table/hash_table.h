...
namespace structures
{
	...
	{
	private:
		static const int DATA_ARRAY_SIZE = 10000;
	public:
	...
	inline T & HashTable<K, T>::operator[](const K key)
	{
		...
		Table<K, T>* tab = (*this->data_)[indexInArray];
		...
		{
			throw std::logic_error("No data for such key!");
		}
	}
	...
	inline void HashTable<K, T>::insert(const K & key, const T & data)
	{
		...
		if (tab == nullptr)
		{
			tab = new LinkedTable<K, T>();
			(*this->data_)[indexInArray] = tab;
		}
	...
	inline T HashTable<K, T>::remove(const K & key)
	{
		...
		if (tab != nullptr)
		{
			T data = tab->remove(key);

			if (tab->isEmpty())
			{
				delete tab;
				(*this->data_)[indexInArray] = nullptr;
			}
	...
	{
		while ((*hashTable_->data_)[position_] == nullptr && position_ < hashTable_->DATA_ARRAY_SIZE)
		{
			position_++;
		}
