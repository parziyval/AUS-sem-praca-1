...
namespace structures
{
	...
	{
		if (PriorityQueueList<T>::list_->isEmpty() || priority < (*PriorityQueueList<T>::list_)[indexOfPeek()]->getPriority()) {
			PriorityQueueList<T>::list_->add(new PriorityQueueItem<T>(priority, data));
		}
		else {
			int index = 0;
			while (priority < (*PriorityQueueList<T>::list_)[index]->getPriority()) {
				index++;
			}
