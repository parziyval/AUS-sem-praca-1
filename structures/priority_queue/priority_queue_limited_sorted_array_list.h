...
namespace structures
{
	...
	inline PriorityQueueItem<T>* PriorityQueueLimitedSortedArrayList<T>::pushAndRemove(const int priority, const T & data)
	{
		if(this->list_->size() < this->capacity_) {
			...
			if (this->list_->size() == this->capacity_) {
				PriorityQueueItem<T>* deleted = (*this->list_)[0];
	...
	inline int PriorityQueueLimitedSortedArrayList<T>::minPriority() const
	{
		...
		for (PriorityQueueItem<T>* item : *this->list_) {
			if (item->getPriority() > minPriorita)
			{
	...
	template<typename T>
	inline int PriorityQueueLimitedSortedArrayList<T>::maxPriority() const
	{
		if (this->list_->size() <= 0)
		{
			return -1;
		...
		return (*this->list_)[this->list_->size() - 1]->getPriority();
	}

	template<typename T>
	inline T PriorityQueueLimitedSortedArrayList<T>::peekData() const
	{
		return (*this->list_)[this->list_->size() - 1]->accessData();
	}
	...
	template<typename T>
	inline size_t PriorityQueueLimitedSortedArrayList<T>::size()
	{
		return this->list_->size();
	}

	template<typename T>
	inline size_t PriorityQueueLimitedSortedArrayList<T>::capacity() const
	{
		return capacity_;
	}
}