...
namespace structures
{
	...
	{
		TreeNode<T>* result = shallowCopy(); //vytvorime shallow copy nody, ktorej strom chceme skopirovat
		...
		for (int i = 0; i < degree(); i++)
		{
			...
			if (son != nullptr)
			{
				result->replaceSon(son->deepCopy(), i); //nahrad svojho syna hlbokou kopiou
			}
		}
		return result;
	}
	...
	{
		if (isRoot()) //ak je root, nema bratov
		{
